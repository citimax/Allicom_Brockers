...
class ClientDivision extends Component {
  constructor() {
    super();
    this.state = {
      Category: [],
    ...
    fetch("api/clientcategory", {
      ...
      .then(Category => {
        if (Category.length > 0) {
          ...
          swal("Oops!", Category.message, "error");
        }
      })
      .catch(err => {
  ...
  handleDelete = Category => {
    swal({
      ...
      if (willDelete) {
        return fetch("api/clientcategory/" + Category.Code, {
          method: "Delete",
          headers: {
            "Content-Type": "application/json",
            "x-access-token": localStorage.getItem("token")
  ...
  handleSubmit = event => {
    ...
    this.postData("api/clientcategory", data);
  };
  postData(url = ``, data = {}) {
    return fetch(url, {
      method: "POST",
  ...
  render() {
    ...
    let Rowdata1 = [];
    const Rows = [...this.state.Category];
