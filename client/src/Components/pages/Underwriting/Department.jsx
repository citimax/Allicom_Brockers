...
class Department extends Component {
    ...
    fetch("api/department", {
      ...
      .then(Department => {
        if (Department.length > 0) {
          ...
          swal("Oops!", Department.message, "error");
        }
      })
      .catch(err => {
  ...
  handleDelete = department => {
    swal({
      ...
      if (willDelete) {
        return fetch("api/department/" + department.Code, {
          method: "Delete",
          headers: {
            "Content-Type": "application/json",
            "x-access-token": localStorage.getItem("token")
  ...
  handleSubmit = event => {
    ...
    this.postData("api/department", data);
  };
  postData(url = ``, data = {}) {
    return fetch(url, {
      method: "POST",
  ...
  render() {
    ...
    let Rowdata1 = [];
    const Rows = [...this.state.Department];
